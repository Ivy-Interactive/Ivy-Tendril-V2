...
import { JobCostSheet, type JobCostFacts } from "./JobCostSheet";
...
const meta: Meta<typeof JobCostSheet> = {
  ...
  args: { isOpen: true, onClose: () => {}, title: "ExecutePlan — 00412" },
};

export default meta;
type Story = StoryObj<typeof JobCostSheet>;

/** A fully reported run: every bucket present, and the breakdown agrees with the total. */
export const FullBreakdown: Story = {
  args: { job: facts() },
};

/**
 * A flat-rate subscription. The `Cost` field is left empty rather than zeroed when a run cannot be
 * priced, so the tokens are still on record without inventing a number.
 */
