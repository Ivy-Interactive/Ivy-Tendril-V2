...
export const JobSessionView: React.FC<JobSessionViewProps> = ({
  ...
  const jsonStream = events.map((e) => JSON.stringify(e.payload)).join("\n");
  ...
  const hasFailed = job.status === "Failed" || job.status === "Timeout" || job.status === "Blocked";
  const noop = () => {};
    ...
    <div className="flex h-full flex-col space-y-4" data-testid="job-session-view">
      ...
      <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm font-semibold text-slate-100">{job.type}</span>
          <span
          ...
          {job.planTitle && <span className="text-xs text-slate-400">Plan: {job.planTitle}</span>}
        </div>
