...
- Review actions MUST bootstrap environment template files if they exist (see Environment Template Bootstrapping below).
...
Detect when `project-analyzer` flags a workspace root (`isWorkspaceRoot: true`) or when the repository root contains monorepo orchestrators (such as Nx via `nx.json`, Turborepo via `turbo.json`, pnpm workspaces via `pnpm-workspace.yaml`, or a root `package.json` with workspace definitions and root `dev` or `start` scripts):
...
1. Check `tendril verification list` for a `Screenshots` definition. If missing, create it. `tendril verification add` takes the prompt inline, so pass the multi-line text through a quoted heredoc to keep backticks and quotes literal:
   ```bash
   tendril verification add Screenshots --prompt="$(cat <<'EOF'
