...
import { mapColorThemeKindToTheme, validateBridgeMessage } from "../../bridge/bridgeProtocol";
...
describe("Tendril IDE Bridge Suite", () => {
  describe("validateBridgeMessage", () => {
    ...
    it("should throw for unknown or malformed messages", () => {
      ...
      assert.throws(() =>
        validateBridgeMessage({ type: "executeCommand", command: "cmd", args: "invalid" }),
      );
      ...
      assert.throws(() =>
        validateBridgeMessage({ type: "startJob", jobType: "RetryPlan", planId: "001" }),
      );
  ...
  describe("BridgeHandler execution", () => {
    ...
    it("should route openWorktree and avoid duplicates", async () => {
      ...
      assert.strictEqual(path.resolve(addedFolders[0].uri.fsPath), path.resolve("/worktree/00284"));
    ...
    it("should route startJob to jobRunner if jobRunner is provided", async () => {
      ...
      assert.deepStrictEqual(updatePlanCalledWith, {
        planId: "00123",
        inst: "Updated instructions",
      });
