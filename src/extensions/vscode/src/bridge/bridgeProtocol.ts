  ...
  switch (obj.type) {
    case "openFile": {
      ...
      return {
        ...
        column:
          typeof obj.column === "number" && Number.isFinite(obj.column) ? obj.column : undefined,
      };
    }
    ...
    case "startJob": {
      ...
      if (typeof obj.jobType !== "string" || !validJobTypes.includes(obj.jobType)) {
        throw new Error(
          `startJob message requires a valid "jobType" (${validJobTypes.join(", ")})`,
        );
      }
      ...
      if (
        obj.jobType === "ExecutePlan" ||
        obj.jobType === "RetryPlan" ||
        obj.jobType === "UpdatePlan"
      ) {
