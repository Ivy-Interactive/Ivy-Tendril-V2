...
const ClaudeCodeIcon = ({ size, color, style, className }: IconProps) => (
  <svg
    width={size || 24}
    height={size || 24}
    viewBox="0 0 256 257"
...
export const Icon: FC<IconProps> = ({ name, color, size, className, style }) => {
  if (name === "None") {
  ...
  if (name === "ClaudeCode") {
    return <ClaudeCodeIcon size={size} color={color} style={style} className={cn(className)} />;
  }
