...
fn apple_opencode_config(base_url: &str) -> String {
    serde_json::json!({
...
fn build_apple_spec(config: &AgentLaunchConfig) -> AgentProcessSpec {
    ...
    // Inserted after the delegate so they beat any caller-supplied environment variables: pointing
    // this agent at a different endpoint is what APPLE_FM_BASE_URL is for.
    spec.environment.insert(
        "OPENCODE_CONFIG_CONTENT".to_string(),
        apple_opencode_config(&base_url),
    );
    spec.environment
