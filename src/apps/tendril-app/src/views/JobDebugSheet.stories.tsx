...
function job(overrides: Partial<JobDetail> = {}): JobDetail {
  return {
    ...
    type: "ExecutePlan",
    planId: "00412",
    planTitle: "Port the dialog stories",
    project: "Ivy-Tendril-V2",
    status: "Completed",
    startedAt: "2026-09-22T09:14:02Z",
    completedAt: "2026-09-22T09:41:55Z",
    provider: "claude",
    workingDirectory: "/home/dev/.tendril/Plans/00412-PortTheDialogStories/Worktrees/main",
    planFolder: "/home/dev/.tendril/Plans/00412-PortTheDialogStories",
    ...overrides,
  };
}
 ...
 * this file. Unlike the dialogs it takes no `isOpen`/`onClose` — it is a sheet *body*, rendered
 * inside a host that owns the open state — so it owes none of `DialogShell`'s contract, and that
 * is why it is listed here rather than under Dialogs.
 *
 * Every field is optional and each is rendered only when present, so the stories are about how much
 * of the table exists at each point in a job's life.
 */
const meta = {
  title: "Sheets/JobDebugSheet",
  component: JobDebugSheet,
  parameters: { layout: "padded" },
} satisfies Meta<typeof JobDebugSheet>;
...
export const CompletedWithArtifacts: Story = {
  args: {
    job: job({
      ...
      args: '{"folderPath":"/home/dev/.tendril/Plans/00412-PortTheDialogStories"}',
...
/**
 * Permission denials. These are the reason `resolve_agent` matters: a launch config built without
 * it gets no tool allow-list, and the run then trips a restrictive permission mode.
 */
...
export const LongArgsBlob: Story = {
  args: {
    job: job({
      args: JSON.stringify(
        {
          folderPath: "/home/dev/.tendril/Plans/00412-PortTheDialogStories",
          ...
          initialPrompt:
            "Give every dialog and sheet a story catalog, and keep the contract runner reading it.",
        },
        null,
        2,
      ),
