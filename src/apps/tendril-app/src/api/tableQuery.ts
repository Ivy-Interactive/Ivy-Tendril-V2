 ...
 *
 * The two are exclusive rather than a fallback chain. A failed `invoke` is a real failure — a daemon
 * that is not running, or a query the daemon rejected naming the column that was wrong — and retrying
 * it over `fetch` inside the shell resolves against the asset origin, so it can only ever replace that
 * reason with a confusing one. `cmd_query_table` (`src-tauri/src/commands/tables.rs`) is the command,
 * and it refuses any path outside the query API rather than proxying it with the daemon's credential.
 */
