...
| Symptom | Fix |
| --- | --- |
| Agent not reachable | Check `codingAgent` in `config.yaml`, then run the agent's own CLI by hand. Tendril shells out to it, so if `claude` (or `codex`, `copilot`, `gemini`, `opencode`) does not run non-interactively for you, it will not run for a job either. |
