...
pub enum VerificationCommands {
    ...
    Add { name: String, #[arg(long, default_value = "")] prompt: String },

    #[command(about = "Remove a verification definition")]
    Remove { name: String },
}

pub fn handle_verification_command(cmd: VerificationCommands, tendril_home: &Path) -> anyhow::Result<()> {
    ...
    match cmd {
        ...
        VerificationCommands::Get { name } => {
            if let Some(v) = settings.verifications.iter().find(|v| v.name.eq_ignore_ascii_case(&name)) {
                println!("Name: {}", v.name);
                println!("Prompt:\n{}", v.prompt);
            } else {
        ...
        VerificationCommands::Add { name, prompt } => {
            if settings.verifications.iter().any(|v| v.name.eq_ignore_ascii_case(&name)) {
                anyhow::bail!("Verification '{}' already exists", name);
            }
            settings.verifications.push(VerificationConfig { name: name.clone(), prompt });
        ...
        VerificationCommands::Remove { name } => {
            ...
            settings.verifications.retain(|v| !v.name.eq_ignore_ascii_case(&name));
